...
!!! info "Singularity → Apptainer"

    [Apptainer][url_apptainer] is the new name for Singularity. In November 2021,
    Singularity joined the Linux Foundation and was renamed to Apptainer. The
...
Apptainer provides the same functionalities as Docker, without any of the
drawbacks listed above. Using a completely different implementation, it doesn't
require any privilege to run containers, and allows direct interaction with
existing Docker containers.


The main motivation to use Apptainer over Docker is the fact that it's been
...
with Apptainer, please see the [Apptainer
documentation][url_apptainer_docs].
...
* [DockerHub][url_docker_hub] contains containers for various software
  ...
  Apptainer][url_apptainer_docker],
...
* many individual projects contain specific instructions for installation via
  Docker and/or Apptainer, and may provide pre-built images in other
  locations.
...
!!! info "Interactive or batch usage"
    ...
    This example shows how to use Apptainer interactively, but Apptainer
    containers can be run in batch jobs as well.

The first step is to request an interactive shell. Apptainer images can be
pulled directly on compute nodes, and Apptainer uses multiple CPU cores
...
The `apptainer shell` command can be used to start the container, and run a
...
!!! note
    ...
    The `APPTAINER_DOCKER_USERNAME` environment variable must be set to the
...
The general form of the Apptainer command used to pull NGC containers is: `$
apptainer pull docker://nvcr.io/<registry>/<app:tag>`
...
After this command has finished, we'll have a Apptainer image file in the
...
> Containers that lack Apptainer documentation have not been tested with
Apptainer.

Since all NGC containers are optimized for GPU acceleration, they will always
be executed with the `--nv` Apptainer option, to enable GPU support within
the container.
...
Apptainer namd-2.12-171025.simg:/tmp/namd_test>
```
...
For complete details about how to build Apptainer containers, please refer to
the [Apptainer documentation][url_apptainer_build].
